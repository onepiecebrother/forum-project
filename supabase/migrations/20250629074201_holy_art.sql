/*
  # Elite Forum - Complete Database Schema

  1. Core Tables
    - `profiles` - User profiles with roles and stats
    - `categories` - Forum categories for organizing threads
    - `threads` - Discussion threads
    - `posts` - Replies to threads
    - `site_settings` - Global site configuration

  2. Verification & Admin System
    - `verification_requests` - User verification requests
    - `admin_requests` - Admin privilege requests

  3. Agent System
    - `agents` - Agent profiles for companion services

  4. Deal System
    - `deals` - Deal negotiations between users
    - `deal_responses` - Responses to deal proposals
    - `deal_reviews` - Reviews after completed deals
    - `review_assessments` - Appeals for unfair reviews
...
CREATE INDEX idx_agents_location ON agents(current_location);
CREATE INDEX idx_deals_initiator_id ON deals(initiator_id);
CREATE INDEX idx_deals_recipient_id ON deals(recipient_id);
CREATE INDEX idx_deal_reviews_reviewee_id ON deal_reviews(reviewee_id);
CREATE INDEX idx_deal_reviews_created_at ON deal_reviews(created_at DESC);
CREATE INDEX idx_deal_reviews_deal_id ON deal_reviews(deal_id);
CREATE INDEX idx_review_assessments_review_id ON review_assessments(review_id);
CREATE INDEX idx_review_assessments_user_id ON review_assessments(user_id);
CREATE INDEX idx_review_assessments_status ON review_assessments(status);
...
-- Insert default data
INSERT INTO site_settings (site_title) VALUES ('Elite Forum');
